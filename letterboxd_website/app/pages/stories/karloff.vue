<template>
  <div class="story" :class="{ 'reduced-motion': reducedMotion }">
    <KarloffCover :meta="data?.meta" @visible="onCoverVisible" />
    ...
    <section class="coda" data-coda>
      <div class="coda-inner">
        <div class="coda-links">
          ...
          <NuxtLink to="/constellations/boris-karloff?name=Boris%20Karloff" class="coda-link">
...
import KarloffCover from '~/components/stories/KarloffCover.vue'
import KarloffReel from '~/components/stories/KarloffReel.vue'
import { useEraAudio } from '~/composables/useEraAudio'

const { data } = await useFetch('/api/stories/karloff', { key: 'stories:karloff' })
...
const audioTracks = computed(() =>
  eras.value.map((e) => ({
    id: e.id,
    src: `/stories/karloff/audio/${e.audioTrack}`,
    label: e.audioLabel,
  }))
)
...
useHead({ title: 'Boris Karloff · Stories' })
</script>
...
.coda {
  ...
  background: linear-gradient(180deg, #07090a 0%, #0c1410 100%);
}
...
.coda-link:hover { color: #c8d8c0; border-color: rgba(200,216,192,0.7); }
...
.audio-toggle {
  ...
  border: 1px solid rgba(200,216,192,0.18);
  border-radius: 999px;
...
.audio-toggle:hover { color: rgba(200,216,192,0.95); border-color: rgba(200,216,192,0.35); }
.audio-toggle.on { color: #64b478; border-color: rgba(100,180,120,0.5); }
.audio-toggle .dot {
...
.audio-toggle.on .dot { background: #64b478; border-color: #64b478; }
