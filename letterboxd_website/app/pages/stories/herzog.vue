<template>
  <div class="story" :class="{ 'reduced-motion': reducedMotion }">
    <HerzogCover :meta="data?.meta" @visible="onCoverVisible" />
    ...
    <section class="coda" data-coda>
      <div class="coda-inner">
        <div class="coda-links">
          ...
          <NuxtLink to="/constellations/werner-herzog?name=Werner%20Herzog" class="coda-link">
...
import HerzogCover from '~/components/stories/HerzogCover.vue'
import HerzogReel from '~/components/stories/HerzogReel.vue'
import { useEraAudio } from '~/composables/useEraAudio'

const { data } = await useFetch('/api/stories/herzog', { key: 'stories:herzog' })
...
const audioTracks = computed(() =>
  eras.value.map((e) => ({
    id: e.id,
    src: `/stories/herzog/audio/${e.audioTrack}`,
    label: e.audioLabel,
  }))
)
...
useHead({ title: 'Werner Herzog · Stories' })
</script>
...
.coda {
  ...
  background: linear-gradient(180deg, #07090a 0%, #0c1814 100%);
}
...
.coda-link {
  ...
  color: #3ab450;
  padding-bottom: 4px;
  border-bottom: 1px solid rgba(58,180,80,0.4);
  transition: color 200ms ease, border-color 200ms ease;
}
.coda-link:hover { color: #d4e8d8; border-color: rgba(212,232,216,0.7); }
...
.audio-toggle:hover { color: rgba(212,232,216,0.95); border-color: rgba(212,232,216,0.35); }
.audio-toggle.on { color: #3ab450; border-color: rgba(58,180,80,0.5); }
.audio-toggle .dot {
...
.audio-toggle.on .dot { background: #3ab450; border-color: #3ab450; }
